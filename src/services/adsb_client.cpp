...
/**
 * Collects the decoded HTTP body. HTTPClient::writeToStream() already unwraps
 * both Content-Length and Transfer-Encoding: chunked; reading getStream()
 * directly does not, so Cloudflare's chunk size lines (and a trailing "0")
 * were passed to ArduinoJson as InvalidInput.
 */
class PollingBodySink : public Stream {
 public:
  explicit PollingBodySink(String& dest) : dest_(dest) {}
...
bool readResponseBodyWithPoll(HTTPClient& http, String& payload,
  ...
  if (stream != nullptr && timeout_ms > 0) {
    stream->setTimeout(timeout_ms);
  }

  PollingBodySink sink(payload);

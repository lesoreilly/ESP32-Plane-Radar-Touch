...
WiFiManagerParameter s_param_bearing(
    "top_bearing", "Top bearing (deg, phone compass, 0=N)", "0", 10,
...
void refreshPortalParamDefaults() {
  ...
  char bearing_buf[16];
  snprintf(bearing_buf, sizeof(bearing_buf), "%.0f",
           services::radar_rotation::topHeading());
...
void onPortalParamsSaved() {
  ...
  if (s_param_bearing.getValue() != nullptr &&
      s_param_bearing.getValue()[0] != '\0') {
    services::radar_rotation::setTopHeading(
        String(s_param_bearing.getValue()).toFloat());
  }
...
void savePortalParamsFromRequest(WebServer& web) {
  ...
  const String top_bearing = web.arg("top_bearing");

  if (!services::location::saveFromStrings(latitude.c_str(),
                                           longitude.c_str())) {
    Serial.println("Invalid lat/lon in portal — keeping previous location");
  }
  if (top_bearing.length() > 0) {
    services::radar_rotation::setTopHeading(top_bearing.toFloat());
  }
